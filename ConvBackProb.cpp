...
{
    vector<Volume> filtergrades = FilterGrades(ACprev, dZC);
    for(int i = 0;i<filtergrades.size();i++)
        filtergrades[i].print();
    vector<Volume> RotatedFilters(filters->size());
    RotateAllVolumes(filters, &RotatedFilters);
    vector<Volume> dACprev = FullConvolution(dZC, &RotatedFilters);

}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
vector<Volume> FullConvolution(vector<Volume>* dZC, vector<Volume>* RotatedFilters)
{
    int p = (((*RotatedFilters)[0])[0])->Rows() - 1;
    vector<Volume> Padded_dZC(dZC->size());
    PadAllVolumes(dZC, &Padded_dZC, p, 0);
...
vector<Volume> RearrangeFilters(vector<Volume>* filter)
{
    ...
    int NumOfFilters = (*filter)[0].size();
    vector<Volume> Result(NumOfFilters);
    for (int i = 0; i < NumOfFilters; i++)
		Result[i].resize(FilterSize);
    for(int i = 0; i < NumOfFilters; i++)
    {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
vector<Volume> FilterGrades(vector<Volume>* ACprev, vector<Volume>* dZC)
{
	...
	for (int i = 0; i<m; i++)
		...
		{
			//convolve the ith activations with the jth filter to produce z which is pointer to (nh,nw) matrix
			Matrix* z = convolve((*ACprev)[i], (*dZC)[j], 1);
            //z is pointer to the output of convolution, push it into the volume A[i]
			A[i][j] = z;
		}
	return A;
