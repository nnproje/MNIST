...
public:
    ...
    void continueTrain(const Matrix& X, Matrix& Y, Matrix& X_dev, Matrix& Y_dev, float alpha, int numOfEpochs, int minibatchSize, string optimizer1,int Numprint, string ET,float lambda,bool batchNorm, bool dropout,float* keep_prob);



private:
    // Feed forward
    Matrix feedforward(const Matrix& x, layer* layers,int L,bool batchNorm, bool dropout,float* keep_prob,string mode);
    // Back propagation
